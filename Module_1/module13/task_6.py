...
def get_dangerous_level(x):
  return x ** 3 - 3 * (x ** 2) - 12 * x + 10


def get_safe_depth(danger_lvl): 
  min_depth = 0 
  max_depth = 4 
  result = (min_depth + max_depth) / 2 
  while abs(get_dangerous_level(result)) >= danger_lvl:     
    if abs(get_dangerous_level(min_depth)) < abs(get_dangerous_level(max_depth)): 
      max_depth = result 
    else: 
      min_depth = result 
    result = (min_depth + max_depth) / 2       
...
safe_level = float(input('Введите максимально допустимый уровень опасности: ')) 
x = get_safe_depth(safe_level) 
