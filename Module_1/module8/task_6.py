...
expense = int(input("Введите расходы на проживание: "))
over_money = 0
for x in range(10):
  if educational_grant < expense:
    over_money += expense - educational_grant
    print(f"{x+1}. месяц траты {round(expense, 1)} не хватает {round(over_money, 2)}")
  else:
    print(f"{x+1}. месяц траты {round(expense, 1)} хватает")
  expense *= 1.03


print(f"Нужно попросить у родителей {round(over_money, 2)} рублей")
